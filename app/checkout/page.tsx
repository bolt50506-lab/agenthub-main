...
export default function CheckoutPage() {
  ...
  useEffect(() => { if (countryCode !== 'PK') setPaymentMethod('bank_transfer'); }, [countryCode]);
  useEffect(() => {
    let active = true;
    fetch('/api/public/plans', { cache: 'no-store' }).then(async (response) => {
      const result = await response.json();
      if (!response.ok) throw new Error(result?.error || 'Unable to load plans');
      const rows = (result?.plans as SubscriptionPlan[]) ?? [];
      if (!active) return;
      setPlans(rows); setPlan(rows.find((item) => item.slug === selectedSlug) || rows[0] || null);
    }).catch((err) => { console.error('[Checkout] Unable to load plans:', err); if (active) { setPlans([]); setPlan(null); } }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [selectedSlug]);
  ...
  async function startCheckout(event: FormEvent) {
    ...
    try {
      const response = await fetch('/api/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ planSlug: plan.slug, customerName, customerEmail, businessName, whatsappNumber, password, countryCode, paymentMethod, billingCycle }) });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || 'Unable to start checkout');
      window.location.href = result.checkoutUrl;
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to start checkout'); setSubmitting(false); }
  }
  ...
  return <main className="min-h-screen bg-[#070b18] text-white">
    ...
    <div className="mx-auto grid max-w-6xl gap-8 px-4 py-10 lg:grid-cols-[1.05fr_.95fr] lg:py-16 sm:px-6">
      <section>
        ...
        <p className="mt-3 max-w-xl text-slate-400">Enter your account details, then pay manually and upload your payment screenshot. Your account and selected plan activate after admin payment approval.</p>
        <form onSubmit={startCheckout} className="mt-8 space-y-5 rounded-2xl border border-white/10 bg-white/[0.025] p-5 sm:p-7">
          ...
          <div><Label className="text-slate-300">WhatsApp number</Label><Input required type="tel" value={whatsappNumber} onChange={(e) => setWhatsappNumber(e.target.value)} className="mt-2 border-white/10 bg-white/[0.04] text-white" placeholder="e.g. 923001234567" /><p className="mt-2 text-xs text-slate-500">Use the same WhatsApp number that will send your payment screenshot. AgentHub uses this verified number to identify your order and release access only after approval.</p></div>
          <div><Label className="text-slate-300">Choose account password</Label><Input required minLength={8} type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-2 border-white/10 bg-white/[0.04] text-white" placeholder="Minimum 8 characters" /><p className="mt-2 text-xs text-slate-500">Your password is encrypted while the payment is pending and is used only to create your account after approval.</p></div>
          ...
          <div><Label className="text-slate-300">Payment method</Label><div className="mt-2 grid gap-2 sm:grid-cols-3">{(countryCode === 'PK' ? [['jazzcash','JazzCash'],['easypaisa','Easypaisa'],['bank_transfer','Bank Alfalah']] : [['bank_transfer','International Bank Transfer']]).map(([value,label]) => <button key={value} type="button" onClick={() => setPaymentMethod(value)} className={`rounded-xl border px-3 py-3 text-sm font-medium transition ${paymentMethod===value ? 'border-violet-400 bg-violet-500/20 text-white' : 'border-white/10 bg-white/[0.03] text-slate-300 hover:border-white/25'}`}>{label}</button>)}</div><p className="mt-2 text-xs text-slate-500">{countryCode === 'PK' ? 'Pay with Easypaisa, JazzCash, or Bank Alfalah.' : 'International orders use bank transfer.'}</p></div>
      ...
      <aside className="lg:sticky lg:top-24 lg:h-fit"><div className="rounded-2xl border border-violet-500/30 bg-gradient-to-b from-violet-500/10 to-white/[0.02] p-6"><p className="text-sm text-violet-200">YOUR SELECTED PLAN</p><h2 className="mt-2 text-3xl font-bold">{plan.name}</h2><p className="mt-3 text-sm leading-6 text-slate-400">{plan.description || 'AI automation for customer conversations.'}</p><div className="mt-6 flex items-end gap-1 border-b border-white/10 pb-6"><span className="text-5xl font-bold">USD ${price}</span><span className="mb-2 text-sm text-slate-500">/{plan.billing_period}</span></div><p className="mt-3 text-xs text-slate-500">Pakistan customers will see the live PKR equivalent before payment.</p><ul className="mt-6 space-y-3">{plan.features.map((feature) => <li key={feature} className="flex gap-2 text-sm text-slate-300"><Check className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />{feature}</li>)}</ul>{billingCycle === 'yearly' && <div className="mt-4 rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-3 text-center text-xs font-semibold text-emerald-200">{annualDiscount}% yearly discount applied</div>}<div className="mt-6 rounded-xl border border-violet-400/15 bg-violet-500/[0.05] p-4 text-xs text-slate-300">After you pay, send the screenshot from the WhatsApp number above. It will appear in Payment Approvals for admin review.</div></div></aside>
    </div>
  </main>;
}
