...
export default function AdminPaymentsPage() {
  ...
  async function load() {
    if (!session?.access_token) { setPayments([]); setVerifications([]); setError('Your admin session is not ready. Please refresh or sign in again.'); setLoading(false); return; }
    ...
    try {
      ...
      if (verificationsResponse.ok) setVerifications(verificationsData?.verifications || []);
      const errors = [
    ...
    <section className="space-y-4">
      ...
      {verifications.length === 0 ? <Card><CardContent className="p-6 text-center text-muted-foreground">No WhatsApp payment proofs yet.</CardContent></Card> :
        <div className="grid gap-4">{verifications.map((v) => <Card key={v.id}>
          ...
          {v.status === 'pending_review' && <div className="flex gap-2 lg:col-span-2"><Button disabled={acting===v.id} onClick={() => actVerification(v.id,'approve')} className="bg-emerald-600 hover:bg-emerald-700"><CheckCircle2 className="mr-2 h-4 w-4" /> Approve Verification</Button><Button disabled={acting===v.id} variant="destructive" onClick={() => actVerification(v.id,'reject')}><XCircle className="mr-2 h-4 w-4" /> Reject</Button></div>}
