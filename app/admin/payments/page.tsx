...
export default function AdminPaymentsPage() {
  const [payments, setPayments] = useState<Payment[]>([]); const [loading, setLoading] = useState(true); const [acting, setActing] = useState(''); const [error, setError] = useState(''); const { session } = useAuth();
  async function load() {
    if (!session?.access_token) { setPayments([]); setError('Your admin session is not ready. Please refresh or sign in again.'); setLoading(false); return; }
    setLoading(true); setError('');
    try { const r = await fetch('/api/admin/payments', { cache: 'no-store', headers: { Authorization: `Bearer ${session.access_token}` } }); const data = await r.json().catch(() => null); if (r.ok) setPayments(data?.payments || []); else setError(data?.error || `Unable to load payments (HTTP ${r.status}).`); } catch { setError('Unable to connect to the payment approvals service.'); } finally { setLoading(false); }
  }
  ...
  async function act(orderId: string, action: 'approve' | 'reject') {
    ...
    setActing(orderId); if (!session?.access_token) { alert('Your admin session is not ready. Please sign in again.'); setActing(''); return; }
    const r = await fetch('/api/admin/payments', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ orderId, action }) }); const data = await r.json().catch(() => null); if (!r.ok) alert(data?.error || 'Action failed'); await load(); setActing('');
  }
  ...
  return <div className="space-y-6">
    <div><h2 className="text-2xl font-bold">Payment Approvals</h2><p className="text-sm text-muted-foreground mt-1">Review payment screenshots. The WhatsApp number shown here is the verified sender identity used to match the order.</p></div>
    {error && <Card className="border-red-500/40"><CardContent className="p-4 text-sm text-red-500">{error}</CardContent></Card>}
    {payments.length === 0 ? <Card><CardContent className="p-8 text-center text-muted-foreground">{error ? 'Payment records could not be loaded.' : 'No submitted payment proofs.'}</CardContent></Card> : <div className="grid gap-4">{payments.map((p) => <Card key={p.id}>
