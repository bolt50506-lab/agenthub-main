  ...
  const setHumanMode = async (human: boolean) => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        ai_resume_at: human ? new Date(Date.now() + 2 * 60 * 1000).toISOString() : null,
      })
      ...
      <div className="grid min-h-0 grid-cols-1 gap-4 xl:h-[calc(100vh-11rem)] xl:grid-cols-[380px_minmax(0,1fr)]">
        <Card className={`flex min-h-[420px] flex-col overflow-hidden xl:min-h-0 ${mobileDetailOpen ? 'hidden md:flex' : 'flex'}`}>
          <CardHeader className="space-y-3 pb-3">
            ...
            <div className="flex gap-2">{[['all','All'],['website_chat','Website'],['whatsapp','WhatsApp'],['facebook_messenger','Facebook'],['instagram','Instagram']].map(([value,label]) => <Button key={value} variant={channel === value ? 'default' : 'outline'} size="sm" onClick={() => setChannel(value as typeof channel)} className="text-xs">{label}</Button>)}</div>
          </CardHeader>
          <CardContent className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-0">
            {filteredConversations.length === 0 ? <div className="px-6 py-14 text-center"><MessageSquare className="mx-auto mb-3 h-10 w-10 text-muted-foreground/40" /><p className="text-sm font-medium">No conversations yet</p><p className="mt-1 text-xs text-muted-foreground">Messages from the website widget, WhatsApp, Facebook Messenger and Instagram will appear here automatically.</p></div> :
        ...
        <Card className={`flex min-h-[520px] flex-col overflow-hidden xl:min-h-0 ${mobileDetailOpen ? 'flex' : 'hidden md:flex'}`}>
          {selectedConversation ? <>
            ...
            <CardContent className="flex min-h-0 flex-1 flex-col p-0">
              ...
              <div className="shrink-0 border-t bg-card p-3"><p className="mb-2 text-[11px] text-muted-foreground">{selectedConversation.human_takeover ? 'Human mode is active. AI will stay silent until you choose Resume AI.' : selectedConversation.channel === 'website_chat' ? 'AI is replying automatically. Send a team reply to take over this conversation.' : 'AI is replying automatically. Send a manual WhatsApp reply to take over this conversation.'}</p>{sendError && <p className="mb-2 rounded-md bg-destructive/10 px-2 py-1.5 text-xs text-destructive">{sendError}</p>}<div className="flex gap-2"><Input value={replyText} onChange={(e) => setReplyText(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendManualReply(); } }} placeholder={selectedConversation.channel === 'whatsapp' ? 'Reply on WhatsApp...' : selectedConversation.channel === 'facebook_messenger' ? 'Reply on Facebook...' : selectedConversation.channel === 'instagram' ? 'Reply on Instagram...' : 'Type a team reply...'} disabled={sending} /><Button onClick={sendManualReply} disabled={!replyText.trim() || sending}>{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</Button></div></div>
            </CardContent>
