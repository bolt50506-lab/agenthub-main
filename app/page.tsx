...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#050816] text-white">
      ...
      <section id="pricing" className="py-20"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="mx-auto max-w-3xl text-center"><p className="text-sm font-semibold uppercase tracking-[.2em] text-cyan-300">Simple pricing</p><h2 className="mt-3 text-3xl font-bold sm:text-5xl">Choose the automation level you need.</h2><p className="mt-5 text-lg text-slate-400">Start small and upgrade as your business grows.</p><div className="mx-auto mt-7 inline-flex rounded-full border border-white/10 bg-white/[.04] p-1"><button onClick={()=>setBillingCycle('monthly')} className={`rounded-full px-5 py-2 text-sm ${billingCycle==='monthly'?'bg-white text-slate-900':'text-slate-300'}`}>Monthly</button><button onClick={()=>setBillingCycle('yearly')} className={`rounded-full px-5 py-2 text-sm ${billingCycle==='yearly'?'bg-white text-slate-900':'text-slate-300'}`}>Yearly</button></div></div><div className="mt-12 grid gap-5 lg:grid-cols-3">{plansLoaded && displayPlans.map(plan=>{const discount=annualDiscount(plan.slug); const monthly=Number(plan.price_cents ?? 0)/100; const yearly=Number(plan.yearly_price_cents ?? monthly*12*(1-discount/100))/100; const price=billingCycle==='yearly'?yearly:monthly; return <div key={plan.id} className="rounded-3xl border border-white/10 bg-white/[.025] p-7"><div className="flex items-center justify-between"><h3 className="text-xl font-bold">{plan.name}</h3>{billingCycle==='yearly'&&<span className="rounded-full bg-emerald-500/10 px-3 py-1 text-xs text-emerald-300">Save {discount}%</span>}</div><div className="mt-7 flex items-end gap-1"><span className="text-4xl font-bold">{plan.currency || 'PKR'} {price.toLocaleString()}</span><span className="pb-1 text-slate-500">/{billingCycle==='yearly'?'year':'month'}</span></div><Link href={`/checkout?plan=${plan.slug}&cycle=${billingCycle}`}><Button className="mt-7 w-full rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600">Choose {plan.name}<ChevronRight className="ml-2 h-4 w-4"/></Button></Link></div>})}</div></div></section>

      <footer className="border-t border-white/10 py-10"><div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 text-sm text-slate-500 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8"><div>© {new Date().getFullYear()} AgentHub AI. Automate • Connect • Grow.</div><div className="flex gap-5"><Link href="/login" className="hover:text-white">Sign in</Link><a href="#pricing" className="hover:text-white">Pricing</a></div></div></footer>
    </main>
  );
}
