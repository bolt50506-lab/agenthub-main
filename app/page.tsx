      ...
      <section id="channels" className="border-b border-white/5 bg-[#0a0f20] py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {[['WhatsApp','Reply to customer messages, answer product questions, capture leads, follow up and support voice-enabled conversations.',MessageCircle,'from-emerald-500/35 via-emerald-500/12 to-[#0d1428]','bg-emerald-500/20 text-emerald-200','WhatsApp automation'],['Instagram','Respond to incoming customer messages and keep social conversations connected to your business workflow.',Instagram,'from-pink-500/35 via-fuchsia-500/12 to-[#0d1428]','bg-pink-500/20 text-pink-200','Instagram DMs'],['Facebook','Bring Messenger conversations into the same AgentHub workspace as your other customer channels.',Facebook,'from-blue-500/35 via-blue-500/12 to-[#0d1428]','bg-blue-500/20 text-blue-200','Facebook Messenger'],['Website Chat','Install the AgentHub widget and turn website visitors into live conversations, leads and opportunities.',Globe2,'from-violet-500/35 via-indigo-500/12 to-[#0d1428]','bg-violet-500/20 text-violet-200','Website widget']].map(([name, desc, Icon, gradient, iconStyle, tag]) => { const I=Icon as typeof MessageCircle; return <Card key={name as string} className={`group border-white/15 bg-gradient-to-b ${gradient as string} text-white shadow-xl shadow-black/20 transition duration-300 hover:-translate-y-1 hover:border-white/25`}><CardContent className="p-6"><div className={`mb-6 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/15 ${iconStyle as string}`}><I className="h-7 w-7" /></div><h3 className="text-xl font-bold">{name as string}</h3><p className="mt-3 text-sm leading-6 text-slate-200">{desc as string}</p><p className="mt-6 text-xs font-semibold text-white">{tag as string} <ArrowRight className="ml-1 inline h-3 w-3 transition group-hover:translate-x-1" /></p></CardContent></Card>})}
          </div>
        </div>
      </section>
