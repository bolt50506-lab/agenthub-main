      ...
      <section id="channels" className="border-b border-white/5 bg-[#0a0f20] py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {[['WhatsApp','Reply to customer messages, answer product questions, capture leads, follow up and support voice-enabled conversations.',MessageCircle,'from-emerald-500/20 to-emerald-500/5','WhatsApp automation'],['Instagram','Respond to incoming customer messages and keep social conversations connected to your business workflow.',Instagram,'from-pink-500/20 to-fuchsia-500/5','Instagram DMs'],['Facebook','Bring Messenger conversations into the same AgentHub workspace as your other customer channels.',Facebook,'from-blue-500/20 to-blue-500/5','Facebook Messenger'],['Website Chat','Install the AgentHub widget and turn website visitors into live conversations, leads and opportunities.',Globe2,'from-violet-500/20 to-indigo-500/5','Website widget']].map(([name, desc, Icon, gradient, tag]) => { const I=Icon as typeof MessageCircle; return <Card key={name as string} className={`border-white/10 bg-gradient-to-b ${gradient as string} text-white shadow-none`}><CardContent className="p-6"><div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-white/10"><I className="h-6 w-6" /></div><h3 className="text-xl font-semibold">{name as string}</h3><p className="mt-3 text-sm leading-6 text-slate-300">{desc as string}</p><p className="mt-6 text-xs font-medium text-white/80">{tag as string} <ArrowRight className="ml-1 inline h-3 w-3" /></p></CardContent></Card>})}
          </div>
        </div>
      </section>
