...
function authorized(req: NextRequest) {
  ...
  const matchedSecret = (Boolean(followupSecret) && token === followupSecret && 'FOLLOWUP_CRON_SECRET') || (Boolean(cronSecret) && token === cronSecret && 'CRON_SECRET') || (Boolean(webhookSecret) && token === webhookSecret && 'AGENTHUB_WEBHOOK_SECRET');
  ...
  for (const candidate of due ?? []) {
    ...
    try {
      ...
      try {
        ...
        const permanentInputError = /Lead has no phone number for WhatsApp follow-up|Automated delivery for this channel is not connected yet/i.test(message);
        await supabase.from('follow_up_tasks').update({ status: permanentInputError ? 'cancelled' : 'pending', last_error: message }).eq('id', task.id).eq('status', 'processing');
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Unknown follow-up processing error';
      failures.push(task.id);
