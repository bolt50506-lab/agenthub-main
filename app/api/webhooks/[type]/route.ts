  ...
  const events = entries.flatMap((entry: any) => {
    ...
    return changes.flatMap((change: any) => {
      ...
      return messages.map((message: any) => ({
        entryId: String(entry?.id || ''),
        senderId: String(message?.from?.id || value?.from?.id || ''),
        recipientId: String(message?.to?.id || value?.to?.id || value?.recipient?.id || entry?.id || ''),
    ...
    const eventCount = entries.reduce((count: number, entry: any) => {
      ...
      const changes = Array.isArray(entry?.changes)
        ? entry.changes.reduce((n: number, change: any) => {
            const value = change?.value || {};
            return n + (Array.isArray(value?.messages) ? value.messages.length : (value?.message ? 1 : 0));
          }, 0)
        : 0;
