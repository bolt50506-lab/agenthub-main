...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!planSlug || !customerName || !customerEmail || !businessName || !whatsappNumber || password.length < 8) return NextResponse.json({ error: 'Please complete all checkout fields, including your WhatsApp number. Password must be at least 8 characters.' }, { status: 400 });
    if (whatsappNumber.length < 8 || whatsappNumber.length > 15) return NextResponse.json({ error: 'Please enter a valid WhatsApp number.' }, { status: 400 });
    ...
    if (countryCode !== 'PK' && paymentMethod !== 'bank_transfer') return NextResponse.json({ error: 'International checkout currently uses bank transfer only.' }, { status: 400 });

    const supabase = createServiceClient();
    const { data: plan, error: planError } = await supabase.from('subscription_plans').select('*').eq('slug', planSlug).eq('is_active', true).maybeSingle();
    ...
    const baseAmountCents = billingCycle === 'yearly' ? Math.round(monthlyUsdCents * 12 * (1 - discount / 100)) : monthlyUsdCents;
    ...
    const amountCents = isPakistan ? Math.round((baseAmountCents / 100) * Number(exchangeRate) * 100) : baseAmountCents;
    ...
    const { data: order, error: orderError } = await supabase.from('public_checkout_orders').insert({
      order_number: orderNumber, plan_id: plan.id, billing_cycle: billingCycle,
      customer_name: customerName, customer_email: customerEmail, business_name: businessName,
      whatsapp_number: whatsappNumber, country_code: countryCode, encrypted_password: encryptPassword(password),
      currency, amount_cents: amountCents, base_amount_cents: baseAmountCents, exchange_rate: exchangeRate,
      payment_method: paymentMethod, status: 'pending_payment', gateway: 'manual',
    }).select('id').single();
    if (orderError || !order) throw new Error(orderError?.message || 'Unable to create payment order');

    return NextResponse.json({ checkoutUrl: `/payment/manual?order=${encodeURIComponent(orderNumber)}`, orderNumber });
  } catch (error) {
