    ...
    if (agent) {
      ...
      if (providerRows && providerRows.length > 0) {
        ...
        if (products && products.length > 0) {
          const productText = products.map((p) => `- ${p.name}${p.description ? `: ${p.description}` : ''}${p.price != null ? ` (Price: ${p.price} ${p.currency})` : ''}${p.availability ? ` [${p.availability}]` : ''}`).join('\n');
          systemPrompt += `\n\nProducts/Services:\n${productText}`;
        }

        if (subscriptionPlans && subscriptionPlans.length > 0) {
          const planText = subscriptionPlans.map((plan) => {
            const currentPrice = typeof plan.price_cents === 'number' ? `${(plan.price_cents / 100).toFixed(2)} ${plan.currency || ''}` : 'Not provided';
            const yearlyPrice = typeof plan.yearly_price_cents === 'number' ? `${(plan.yearly_price_cents / 100).toFixed(2)} ${plan.currency || ''}` : 'Not provided';
            const features = Array.isArray(plan.features) && plan.features.length ? plan.features.join(', ') : 'No feature list provided';
        ...
        if (settings?.response_language) systemPrompt += ` Respond in ${settings.response_language}.`;
        ...
        try {
          ...
          const response = await generateAIResponseWithFallback(
            { messages: aiMessages, temperature: 0.7, maxTokens: settings?.max_response_length ?? 512 },
            providerConfigs
          );
