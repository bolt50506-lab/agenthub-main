...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!serviceResponse.ok || serviceData?.success === false) return NextResponse.json({ success: false, error: serviceData?.message || 'WhatsApp provider could not send the message' }, { status: 502 });
    ...
    const { error: takeoverError } = await supabase.from('conversations').update({ human_takeover: true, human_takeover_at: now, ai_resume_at: null, last_message_at: now }).eq('id', conversation_id).eq('business_id', business_id);
    if (takeoverError) console.error('[WhatsApp] Manual reply sent but human takeover state could not be saved:', takeoverError.message);

    return NextResponse.json({ success: true, mode: 'human', human_takeover: true });
  } catch (error) {
