...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: media } = await supabase
      ...
      .select('id,file_path,file_type,mime_type,category')
      .eq('id', mediaId)
      .eq('business_id', businessId)
      .maybeSingle();
    if (!media) return NextResponse.json({ error: 'Media not found' }, { status: 404 });
    if (media.file_type !== 'image' || !String(media.mime_type || '').startsWith('image/')) {
      return NextResponse.json({ error: 'Only image files can be analyzed' }, { status: 400 });
    }
    ...
    if (!provider?.api_key_encrypted) {
      await supabase.from('image_analysis_results').upsert({
        ...
        media_document_id: mediaId,
        processing_status: 'failed',
        verification_status: 'unverified',
        uncertain_segments: [{ text: '', note: 'Gemini vision provider is not configured.' }],
    ...
    const response = await fetch(`${baseUrl}/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(provider.api_key_encrypted)}`, {
      ...
      body: JSON.stringify({
        contents: [{
          role: 'user',
          parts: [
            { text: `Analyze this image for a business assistant. If it is a prescription, receipt, invoice, product label, document, or handwritten note, extract the visible text as accurately as possible. Preserve Urdu script and Roman Urdu exactly where visible. Never invent unreadable text. Return ONLY JSON: {"extracted_text":"...","confidence":"high|medium|low","uncertain_segments":[{"text":"...","note":"..."}],"summary":"..."}. For medical prescriptions, do not diagnose, recommend treatment, or invent dosages.` },
    ...
    const parsed = JSON.parse(cleanJsonText(output)) as { extracted_text?: string; confidence?: string; uncertain_segments?: unknown[]; summary?: string };
    ...
    return NextResponse.json({ success: true, extracted_text: parsed.extracted_text || '', confidence: parsed.confidence || 'low', uncertain_segments: parsed.uncertain_segments || [], summary: parsed.summary || '' });
  } catch (error) {
