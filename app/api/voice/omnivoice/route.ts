...
const LANGUAGE_ALIASES: Record<string, string> = {
  english: 'en', en: 'en', 'english (us)': 'en', 'english (uk)': 'en',
  urdu: 'ur', 'roman urdu': 'ur', romanurdu: 'ur', ur: 'ur',
  hindi: 'hi', hi: 'hi',
  arabic: 'ar', ar: 'ar',
  punjabi: 'pa', pa: 'pa',
  spanish: 'es', es: 'es', french: 'fr', fr: 'fr',
  german: 'de', de: 'de', italian: 'it', it: 'it',
  portuguese: 'pt', pt: 'pt', chinese: 'zh', zh: 'zh',
  japanese: 'ja', ja: 'ja', korean: 'ko', ko: 'ko',
};

function normalizeLanguage(value: string | null) {
  const raw = (value || '').trim().toLowerCase();
  return LANGUAGE_ALIASES[raw] || raw || 'en';
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!profileResponse.ok || typeof profileData.id !== 'string') {
      return NextResponse.json({ error: typeof profileData.error === 'string' ? profileData.error : 'OmniVoice rejected profile creation' }, { status: 502 });
    }
    ...
    try {
      ...
      sampleForm.append('file', sample, sample.name || 'voice-sample');
    ...
    const agreement = await supabase.from('voice_clone_agreements').insert({
      business_id: businessId,
      ...
      voice_name: name,
      provider: 'omnivoice',
      agreement_version: 'voice-cloning-consent-v1',
