...
async function synthesizeOmniVoice(baseUrl: string, profileId: string, text: string, language: string, instruct?: string) {
  ...
  for (let attempt = 0; attempt < 120; attempt += 1) {
    const audio = await fetch(`${baseUrl}/audio/${encodeURIComponent(generation.id)}`, { signal: AbortSignal.timeout(20_000) }).catch(() => null);
    if (audio?.ok) return { response: audio, error: null };
    const history = await fetch(`${baseUrl}/history/${encodeURIComponent(generation.id)}`, { signal: AbortSignal.timeout(10_000) }).catch(() => null);
    if (history?.ok) {
...
export async function POST(req: NextRequest) {
  ...
  // OmniVoice is the primary self-hosted voice engine. It supports native Urdu,
  // so we no longer force Urdu through Hindi/Devanagari conversion.
  if (voice.provider === 'omnivoice') {
    const baseUrl = (process.env.OMNIVOICE_SERVICE_URL || '').replace(/\/$/, '');
    if (!baseUrl) return NextResponse.json({ error: 'OmniVoice service is not configured' }, { status: 503 });
    ...
    const generated = await synthesizeOmniVoice(
      baseUrl,
      voice.provider_voice_id,
      ttsText,
      ttsLanguage,
      wantsUrdu ? 'natural conversational Pakistani Urdu delivery; warm human pacing; preserve the speaker identity' : undefined,
    );
    ...
    return new Response(audio, {
      status: 200,
      headers: {
        'Content-Type': generated.response.headers.get('content-type') || 'audio/wav',
        'Cache-Control': 'no-store',
        'X-AgentHub-Voice-Profile': voice.id,
        'X-AgentHub-Voice-Provider': 'omnivoice',
  ...
  if (voice.provider === 'voicebox') {
    ...
    const generateResponse = await fetch(`${baseUrl}/generate`, {
      ...
      body: JSON.stringify({
        ...
        ...(wantsUrdu ? { instruct: 'natural conversational Pakistani Urdu delivery' } : {}),
      }),
    }).catch(() => null);
    ...
    return new Response(audio, { status: 200, headers: { 'Content-Type': audioResponse.headers.get('content-type') || 'audio/wav', 'Cache-Control': 'no-store' } });
  }
  ...
  const providerResponse = await fetch(`${baseUrl}/v1/text-to-speech/${encodeURIComponent(voice.provider_voice_id)}?output_format=mp3_22050_32`, {
    ...
    body: JSON.stringify({ text, model_id: config.model || 'eleven_flash_v2_5', voice_settings: { stability: 0.5, similarity_boost: 0.75, style: 0, use_speaker_boost: true, speed: 1 } }),
  });
