...
const SAFE_ACTIONS = new Set([
  'follow_up', 'send_followup', 'capture_lead', 'update_lead',
  'schedule_followup', 'appointment_reminder', 'recover_conversation',
]);
...
export async function POST(request: NextRequest) {
  ...
  let query = supabase.from('ai_operator_actions').select('*').eq('status', 'approved').order('created_at', { ascending: true }).limit(limit);
  ...
  for (const action of actions ?? []) {
    ...
    if (!updateError) executed++;
  }

  return NextResponse.json({ success: true, found: actions?.length ?? 0, executed, skipped });
}
