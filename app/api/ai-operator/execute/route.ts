...
function authorized(request: NextRequest) {
  ...
  return configured.some(secret =>
    bearer === `Bearer ${secret}` || header === secret
...
async function queueFollowup(supabase: ReturnType<typeof getSupabase>, params: {
  businessId: string;
  leadId: string;
...
async function autoDiscover(supabase: ReturnType<typeof getSupabase>, businessId: string, limit: number) {
  ...
  if (settings.auto_appointment_reminders !== false) {
    ...
    for (const a of appointments || []) {
      ...
      actions.push({
        ...
        risk_level: 'low',
        idempotency_key: `appointment-reminder:${a.id}:${a.date}:${a.start_time}`,
  ...
  if (settings.auto_followups) {
    for (const c of conversations || []) {
      ...
      const { data: lead } = await supabase.from('leads')
        .select('id,name,phone,conversation_id,status')
        .eq('business_id', businessId)
  ...
  for (const c of unlinked || []) {
    const { data: existingLead } = await supabase.from('leads')
      .select('id').eq('business_id', businessId).eq('conversation_id', c.id).limit(1).maybeSingle();
  ...
  const { error } = await supabase.from('operator_actions').upsert(actions, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return jsonError(error instanceof Error ? error.message : 'Supabase server configuration is missing', 500);
  }

  try {
    if (businessId) {
      await autoDiscover(supabase, businessId, limit);
    } else {
      const { data: settings } = await supabase.from('operator_settings')
        .select('business_id').eq('mode', 'autonomous').limit(20);

      for (const row of settings || []) {
        await autoDiscover(supabase, row.business_id, Math.min(limit, 20));
      }
    }
  } catch (error) {
  ...
  let query = supabase.from('operator_actions')
  ...
  for (const action of actions || []) {
    ...
    const { data: claimed } = await supabase.from('operator_actions')
    ...
    try {
      ...
      const payload = claimed.payload && typeof claimed.payload === 'object'
        ? claimed.payload as Record<string, any>
        : {};
      ...
      if (type === 'create_lead_followup') {
        ...
        const { data: lead } = await supabase.from('leads')
          .select('id,name,phone,conversation_id,status')
          .eq('id', leadId).eq('business_id', bid).maybeSingle();
        ...
        result = await queueFollowup(supabase, {
          businessId: bid,
          leadId: lead.id,
          conversationId: lead.conversation_id,
          message: text(payload.message, `Hi ${lead.name || ''}! Just checking in to see if you still need any help. 😊`),
        });
      } else if (type === 'prepare_payment_reminder') {
        const { data: order } = await supabase.from('orders')
          .select('id,lead_id,customer_name,balance_due,payment_status,currency')
          .eq('id', claimed.entity_id).eq('business_id', bid).maybeSingle();
        ...
        if (!order.lead_id) {
          ...
          result = await queueFollowup(supabase, {
            ...
            message: `Hi ${order.customer_name || ''}! Just a friendly reminder that your outstanding balance is ${Number(order.balance_due || 0)} ${order.currency || 'PKR'}. Please let us know if you need any help.`.trim(),
            taskType: 'message',
          });
        }
      } else if (type === 'appointment_recovery') {
        const { data: appointment } = await supabase.from('appointments')
          .select('id,lead_id,customer_name,service_name,status,date,start_time')
          .eq('id', claimed.entity_id).eq('business_id', bid).maybeSingle();

        if (!appointment) throw new Error('Appointment no longer exists');
        if (appointment.status !== 'cancelled') {
          result = { skipped: true, reason: 'appointment_no_longer_cancelled' };
        } else if (!appointment.lead_id) {
          result = { skipped: true, reason: 'no_linked_lead' };
        } else {
          result = await queueFollowup(supabase, {
            businessId: bid,
            leadId: appointment.lead_id,
            appointmentId: appointment.id,
            message: `Hi ${appointment.customer_name || ''}! We noticed your ${appointment.service_name ? appointment.service_name + ' ' : ''}appointment was cancelled. Would you like me to help arrange a new time? 😊`.trim(),
            taskType: 'appointment_recovery',
          });
        }
        ...
        if (!appointment) throw new Error('Appointment no longer exists');
        if (!['confirmed', 'scheduled', 'booked'].includes(String(appointment.status))) {
          result = { skipped: true, reason: 'appointment_not_active' };
        } else if (!appointment.lead_id) {
          result = { skipped: true, reason: 'no_linked_lead' };
        } else {
          result = await queueFollowup(supabase, {
            ...
            leadId: appointment.lead_id,
            appointmentId: appointment.id,
            message: `Hi ${appointment.customer_name || ''}! Friendly reminder: your${appointment.service_name ? ` ${appointment.service_name}` : ''} appointment is scheduled for ${appointment.date} at ${appointment.start_time}. Please let us know if you need to reschedule. 😊`.trim(),
        ...
        const { data: conflict } = await supabase.from('appointments')
          .select('id').eq('business_id', bid).eq('date', date)
          .in('status', ['confirmed', 'scheduled', 'booked'])
        ...
        const { data: lead } = await supabase.from('leads')
          .select('id,customer_id,name').eq('id', leadId).eq('business_id', bid).maybeSingle();
        ...
        }).select('id,date,start_time,end_time,status').single();
        ...
        const { data: current } = await supabase.from('appointments')
          .select('id,lead_id,status').eq('id', claimed.entity_id).eq('business_id', bid).maybeSingle();

        if (!current) throw new Error('Appointment no longer exists');
        if (['cancelled', 'completed', 'no_show'].includes(String(current.status))) {
          throw new Error('Appointment cannot be rescheduled in its current status');
        }
        ...
        const { data: conflict } = await supabase.from('appointments')
          .select('id').eq('business_id', bid).eq('date', date).neq('id', current.id)
          .in('status', ['confirmed', 'scheduled', 'booked'])
          .lt('start_time', endTime).gt('end_time', startTime)
          .limit(1).maybeSingle();
        ...
        const { error: updateError } = await supabase.from('appointments')
          ...
          .eq('id', current.id).eq('business_id', bid);
        ...
        const conversationId = text(payload.conversation_id) ||
          (claimed.entity_type === 'conversation' ? claimed.entity_id : '');

        if (!conversationId) throw new Error('conversation_id is required');

        const { data: conversation } = await supabase.from('conversations')
          .select('id,status,human_takeover').eq('id', conversationId).eq('business_id', bid).maybeSingle();

        if (!conversation) throw new Error('Conversation no longer exists');

        const { error: handoffError } = await supabase.from('conversations')
          .update({
            human_takeover: true,
        ...
        await supabase.from('business_events').insert({
          business_id: bid,
          event_type: 'operator_human_handoff',
          entity_type: 'conversation',
          entity_id: conversationId,
          summary: text(payload.reason, 'AI Operator escalated this conversation to a human.'),
          payload: { reason: text(payload.reason), priority: text(payload.priority, 'high') },
        });
        ...
        if (['won', 'lost'].includes(String(lead.status))) {
          ...
          result = await queueFollowup(supabase, {
            ...
            leadId: lead.id,
            conversationId: lead.conversation_id ||
              (claimed.entity_type === 'conversation' ? claimed.entity_id : null),
            message: text(payload.message, `Hi ${lead.name || ''}! Just checking in — is there anything else I can help you with? 😊`),
          });
        }
      } else if (type === 'capture_lead') {
        const { data: conversation } = await supabase.from('conversations')
          .select('id,customer_id,channel').eq('id', claimed.entity_id).eq('business_id', bid).maybeSingle();

        if (!conversation?.customer_id) throw new Error('Conversation has no customer');

        const { data: customer } = await supabase.from('customers')
          .select('id,name,phone,email').eq('id', conversation.customer_id).eq('business_id', bid).maybeSingle();

        if (!customer) throw new Error('Customer no longer exists');

        const { data: existing } = await supabase.from('leads')
          .select('id').eq('business_id', bid).eq('conversation_id', conversation.id)
          .limit(1).maybeSingle();

        if (existing) {
          result = { already_exists: true, lead_id: existing.id };
        } else {
          const { data: lead, error: leadError } = await supabase.from('leads').insert({
            ...
            source: conversation.channel || 'conversation',
            status: 'new',
          }).select('id').single();
        ...
        const { error: updateError } = await supabase.from('leads')
          .update(patch).eq('id', leadId).eq('business_id', bid);
      ...
      const { error: completeError } = await supabase.from('operator_actions')
        .update({
          status: 'completed',
          result,
        ...
        .eq('id', claimed.id).eq('status', 'executing');
      ...
      await supabase.from('operator_actions').update({
        status: 'failed',
