...
def generate_job(job_id, profile, text, language, instruct=None):
    try:
        ...
        config = OmniVoiceGenerationConfig(
            num_step=int(os.environ.get("OMNIVOICE_NUM_STEPS", "32")),
            denoise=True,
            preprocess_prompt=True,
            postprocess_output=True,
        )

        print(f"[OmniVoice] Generating job={job_id} language={language} chars={len(text)}")
        ...
        print(f"[OmniVoice] Completed job={job_id} output={output}")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/history/"):
            job_id = self.path.split("/", 2)[2].split("?", 1)[0]
            self._json(200, jobs.get(job_id, {"status": "pending"}))
            return
        self.send_error(404)
    ...
    def do_DELETE(self):
        if self.path.startswith("/profiles/"):
            profile_id = self.path.split("/", 2)[2].split("?", 1)[0]
            profile = read_profile(profile_id)
            if not profile:
                self._json(404, {"error": "Profile not found"})
                return
            Path(profile.get("reference_audio") or "").unlink(missing_ok=True)
