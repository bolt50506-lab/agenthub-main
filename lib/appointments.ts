...
Rules:
- "wants_appointment" is true ONLY if the customer clearly agreed to or requested a SPECIFIC date AND a specific time.
- If either date or time is vague, missing, or only implied, set wants_appointment to false and confidence to "low".
...
- Never invent a date or time that wasn't stated or clearly implied by the customer.
`.trim();
  ...
  try {
    ...
    if (response.error || !response.content?.trim()) return null;

    const json = extractJsonObject(response.content);
    if (!json) {
      console.warn('[Appointments] AI response did not contain a JSON object; ignoring detection result.');
      return null;
    }
    ...
    try {
      ...
      console.warn('[Appointments] Invalid JSON from AI; ignoring detection result.', parseError);
      return null;
    }

    if (!parsed.wants_appointment || parsed.confidence !== 'high') return null;
    if (!parsed.date || !parsed.start_time || !isValidDate(parsed.date) || !isValidTime(parsed.start_time)) return null;
