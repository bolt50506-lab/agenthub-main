...
function overlaps(start: number, end: number, a: string, b: string) {
  ...
  return start < ah * 60 + am || end > bh * 60 + bm ? (start < bh * 60 + bm && end > ah * 60 + am) : true;
}
...
export async function appointmentConversationReply(supabase: SupabaseClient, businessId: string, customerId: string | null, conversationId: string, text: string) {
  ...
  const slots = await getAvailableAppointmentSlots(supabase, businessId, intent.requestedDate, intent.durationMinutes, intent.requestedTime);
  if (intent.requestedTime && slots.includes(intent.requestedTime)) {
    ...
    const endMinutes = startMinutes + intent.durationMinutes;
    if (endMinutes > CLOSE) return formatSlots(intent.requestedDate, slots);
    ...
    if (conflict) return `Ye slot abhi kisi aur ne book kar liya hai. ${formatSlots(intent.requestedDate, await getAvailableAppointmentSlots(supabase, businessId, intent.requestedDate, intent.durationMinutes))}`;
